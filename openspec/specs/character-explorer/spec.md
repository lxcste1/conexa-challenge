...
- **THEN** it renders an image with the character's image URL
...
- **THEN** character cards are rendered immediately without a loading state
...
- **WHEN** `useCharacterColumn()` is called
- **THEN** it returns `{ characters, info, isLoading, error, page, setPage }`
